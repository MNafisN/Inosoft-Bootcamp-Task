...
class SiswaController extends Controller
{
    ...
    {
        $semuaSiswa = Siswa::select('nama_siswa', 'kelas')->get();
    ...
    public function DetailSiswa(int $noInduk)
    {
        ...
        if (!$siswa) {
            ...
            $raporSiswaMat = $this->HitungNilaiRapor($siswa->rapor_mata_pelajaran[0]['detail_nilai']);
            $raporSiswaBI = $this->HitungNilaiRapor($siswa->rapor_mata_pelajaran[1]['detail_nilai']);
            $raporSiswaBIng = $this->HitungNilaiRapor($siswa->rapor_mata_pelajaran[2]['detail_nilai']);
            $raporSiswaIPA = $this->HitungNilaiRapor($siswa->rapor_mata_pelajaran[3]['detail_nilai']);
            $raporSiswaIPS = $this->HitungNilaiRapor($siswa->rapor_mata_pelajaran[4]['detail_nilai']);

            $nilai = array(
                $siswa->rapor_mata_pelajaran[0]['nama_mapel'] => $raporSiswaMat,
                $siswa->rapor_mata_pelajaran[1]['nama_mapel'] => $raporSiswaBI,
                $siswa->rapor_mata_pelajaran[2]['nama_mapel'] => $raporSiswaBIng,
                $siswa->rapor_mata_pelajaran[3]['nama_mapel'] => $raporSiswaIPA,
                $siswa->rapor_mata_pelajaran[4]['nama_mapel'] => $raporSiswaIPS
            );

            return response()->json([
                'detail_siswa' => array_diff_key($siswa->toArray(), array_flip(['rapor_mata_pelajaran', 'updated_at', 'created_at'])),
                'nilai_siswa' => $nilai
            ]);
        }
    }
